...
TouTiao.ShowList = function (data, pageIndex, categoryId, append) {
    if (PARAM_CHECKER.IsArray(data)) {
        ...
        for (var k = 0; k < data.length; k++) {
            ...
            var itemHtml = html.replace("[Title]", itemData.Title).replace("[Summary]", (true === PARAM_CHECKER.IsNotEmptyString(itemData.Summary))?itemData.Summary:"").replace("[ImageUrl]", itemData.ImageUrl).replace("[CreatorName]", itemData.CreatorName)
                .replace("[ParentName]", itemData.ParentName)
...
TouTiao.LoadArticle =  function (param, callback) {
    ...
    if (false === PARAM_CHECKER.IsNotEmptyString(id)) {
        id = $(event.target).parents("[data-id]").attr("data-id");
    }
    var context = [id];
